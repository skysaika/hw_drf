...
class LessonTestCase(APITestCase):
    ...
    def test_create_course(self):
        url = reverse('app_study:courses-list')  # DefaultRouter создает 'courses-list' для списка/создания
        data = {
            'title': 'New Course',
    ...
    def test_retrieve_course(self):
        url = reverse('app_study:courses-detail', args=[self.course.id])  # DefaultRouter создал 'courses-detail'
        response = self.client.get(url)
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        json_response = response.json()
